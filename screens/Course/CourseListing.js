...
function CourseListing({appTheme, route, navigation}) {
  const {category, sharedElementPrefix} = route.params;
  const headerSharedValue = useSharedValue(80);
  let filterModalSharedValue1 = useSharedValue(SIZES.height);
  let filterModalSharedValue2 = useSharedValue(SIZES.height);
    ...
    return (
      <AnimatedFlatlist
        ...
        ItemSeparatorComponent={() => (
          <LineDivider
            lineStyle={{height: 1, backgroundColor: COLORS.gray20}}
          />
        )}
      />
    );
